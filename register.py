...
class Register:
    ...
    def initialize(self, pyramid_images_output_path=None):
        ...
        for i in range(pyramid_levels):
            ...
            #  对图像先用高斯滤波模糊图像，然后根据factor格点采样[0::factor]
            ...
            # 根据掩膜只将掩膜为true的部分归一化，这里掩膜在整张图片都为true, 所以不知道是干嘛的
            ...
            if pyramid_images_output_path is not None and ref_resampled.ndim == 2:
                ref_resampled_save = cv2.convertScaleAbs(ref_resampled, alpha=(255.0))
                flo_resampled_save = cv2.convertScaleAbs(flo_resampled, alpha=(255.0))
                cv2.imwrite('%sref_resampled_%d.png' % (os.path.join(pyramid_images_output_path, "downsample/"), i+1), ref_resampled_save)
                cv2.imwrite('%sflo_resampled_%d.png' % (os.path.join(pyramid_images_output_path, "downsample/"), i+1), flo_resampled_save)
            ...
            ref_diag = np.sqrt(np.square(np.array(ref_resampled.shape)*self.ref_spacing).sum())   #  计算下采样后的对角线长度      
